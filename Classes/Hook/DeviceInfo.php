...
class DeviceInfo {
	...
	private function readUserAgent() {
		$this->userAgent = \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('HTTP_USER_AGENT');
	}
	
	private function readAcceptHeader() {

		$strHeaderAccept = \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('HTTP_ACCEPT');
		$this->acceptHeaders = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $strHeaderAccept, TRUE);		
	}
	...
	public function getExternalData(&$arrData) {
		...
		$rdf = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('ParseRdf', $this->xmlLink);
	...
	public function getDeviceType($params) {
		...
		if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog(__METHOD__, $this->extKey, 0, $params);
		...
		if ($this->xmlLink !== '') {
			...
			if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Has UAProf header', $this->extKey);
		...
		if (intval($this->extConf['DetectTablets']) != 0) {
			...
			if ($this->is_AndroidTablet($userAgent)) {
				if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Found Android tablet', $this->extKey);
				return 'tablet';
			}
			
			if ($this->is_iPad($userAgent)) {
				if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Found iPad tablet', $this->extKey);
				return 'tablet';
			}
			
			if ($this->is_WindowsTablet($userAgent)) {
				if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Found Windows tablet', $this->extKey);
				return 'tablet';
			}
		}
		
		if ($this->is_AndroidMobile($userAgent)) {
			if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Found Android mobile device', $this->extKey);
			return $this->extConf['DetectedDeviceType'];		
		}
	
		if ( $this->doTest1($userAgent)) {
			if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('doTest1() is TRUE', $this->extKey);
			return $this->extConf['DetectedDeviceType'];
		}

		if (TYPO3_DLOG) \TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Trying TYPO3 method', $this->extKey);
		...
		if ($this->xmlLink !== '') {
			...
			if (empty($this->deviceCaps)) {
				...
				if (!empty($dd)) {
					...
					if (TYPO3_DLOG)
						\TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Device data from xml profile', $this->extKey, 0, $dd);
